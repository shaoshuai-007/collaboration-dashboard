...
# ==================== 配色方案（遵守60-30-10法则）====================
# 主色（60%）
PRIMARY = RGBColor(201, 56, 50)      # 电信红 - 标题、重点、核心元素
PRIMARY_DARK = RGBColor(172, 0, 0)   # 深红 - 标题装饰

# 辅色（30%）
SECONDARY = RGBColor(0, 110, 189)    # 深蓝 - 次要元素、系统、技术
SECONDARY_LIGHT = RGBColor(79, 129, 189)  # 浅蓝

# 中性色（10%）
NEUTRAL_DARK = RGBColor(89, 89, 89)    # 深灰 - 正文
NEUTRAL = RGBColor(166, 166, 166)       # 灰色 - 辅助
NEUTRAL_LIGHT = RGBColor(240, 240, 240) # 浅灰 - 背景
...
def add_layered_architecture(slide, left, top, width, height, layers):
    ...
    for i, (name, items) in enumerate(layers):
        ...
        # 层名称
        add_rounded_box(slide, left, y, width, layer_height * 0.35, color, name, 11, WHITE)
        
        # 层内容（3个要点）
        if items:
            item_y = y + layer_height * 0.4
            item_height = layer_height * 0.18
            item_width = width / len(items) - 0.1
            
            for j, item in enumerate(items):
                item_x = left + j * (item_width + 0.1)
                add_rounded_box(slide, item_x, item_y, item_width, item_height, NEUTRAL_LIGHT, item, 8, NEUTRAL_DARK, False)
...
add_target_line(slide, "架构清晰可落地，六层分层设计，每层职责明确。")
add_measure_title(slide)

items = [
    ("架构理念", "分层设计，每层职责明确，便于开发、测试、运维，支持横向扩展。"),
    ("技术选型", "大模型+RAG技术成熟，开源组件丰富，技术风险低。")
]
add_measure_content(slide, items)
...
add_layered_architecture(slide, 6.2, 2.4, 6.0, 4.5, layers)
...
# 保存
output_path = "/root/.openclaw/workspace/03_项目管理/营业受理数字员工/03_工程输出/指南针工程/营业受理数字员工建设方案_精修版.pptx"
prs.save(output_path)
print(f"\n✅ 精修版PPT已生成: {output_path}")
print(f"总页数: {len(prs.slides)}")
print("\n改进点：")
print("1. 遵守60-30-10配色法则（主色60%、辅色30%、中性色10%）")
print("2. 整体架构每层扩充3点内容")
print("3. 删除公司信息")
print("4. 只用3种主色：电信红、深蓝、灰色")
