...
# 设置中文字体
plt.rcParams['font.sans-serif'] = ['WenQuanYi Zen Hei', 'WenQuanYi Zen Hei Mono', 'DejaVu Sans']
...
def create_overall_arch():
    ...
    # 标题
    ax.text(8, 19.5, 'Token采集系统 - 总体架构图', 
            ha='center', fontsize=18, fontweight='bold')
    
    # Layer 4: 数据应用层
    draw_box(ax, 0.5, 16, 15, 2.5, '', COLORS['primary'])
    ax.text(8, 18.2, '第四层：数据应用层', ha='center', fontsize=12, 
    ...
    ax.text(8, 14.7, '第三层：数据处理层', ha='center', fontsize=12, 
            color='white', fontweight='bold')
    ...
    ax.text(8, 10.7, '第二层：数据采集层', ha='center', fontsize=12, 
            color='white', fontweight='bold')
    ...
    ax.text(8, 5.7, '第一层：数据源层', ha='center', fontsize=12, 
            color='white', fontweight='bold')
    ...
    ax.text(8, 2.3, '支撑体系', ha='center', fontsize=12, 
            color='white', fontweight='bold')
    ...
    plt.tight_layout()
    output_path = os.path.join(OUTPUT_DIR, '总体架构图_中文版.png')
...
def create_deploy_arch():
    ...
    # 标题
    ax.text(8, 11.5, 'Token采集系统 - 部署架构图', 
            ha='center', fontsize=18, fontweight='bold')
    ...
    ax.text(11.75, 10.7, '生产环境', ha='center', fontsize=11, fontweight='bold')
    ...
    plt.tight_layout()
    output_path = os.path.join(OUTPUT_DIR, '部署架构图_中文版.png')
    plt.savefig(output_path, dpi=150, bbox_inches='tight', facecolor='white')
    plt.close()
...
def create_db_arch():
    ...
    # 标题
    ax.text(7, 15.5, 'Token采集系统 - 数据库架构图', 
            ha='center', fontsize=18, fontweight='bold')
    
    # 应用层
    draw_box(ax, 0.5, 12.5, 13, 2, '', COLORS['primary'])
    ax.text(7, 14.3, '应用层 (Doris)', ha='center', fontsize=12, 
            color='white', fontweight='bold')
    ...
    ax.text(7, 11.2, '汇总层 (Hive)', ha='center', fontsize=12, 
            color='white', fontweight='bold')
    ...
    ax.text(7, 7.7, '明细层 (Hive)', ha='center', fontsize=12, 
            color='white', fontweight='bold')
    details = ['Token明细表\ntoken_detail\n分区: data_date', 
               '操作日志表\ntoken_operation_log',
               '错误记录表\ntoken_error_record']
    ...
    ax.text(7, 3.7, '主数据层 (MySQL/Doris)', ha='center', fontsize=12, 
            color='white', fontweight='bold')
    masters = ['单位主数据\ndim_org', '模型主数据\ndim_model', 
               '硬件主数据\ndim_hardware', '应用主数据\ndim_app']
    ...
    plt.tight_layout()
    output_path = os.path.join(OUTPUT_DIR, '数据库架构图_中文版.png')
    plt.savefig(output_path, dpi=150, bbox_inches='tight', facecolor='white')
    plt.close()
