...
def call_qianfan(system_prompt: str, user_message: str, temperature: float = 0.7) -> Optional[str]:
    """调用百度千帆API - 使用正确的endpoint"""
    
    if not QIANFAN_API_KEY:
        return None
    
    try:
        # 使用标准千帆API endpoint (ERNIE-4.0-8K)
...
<body>
    ...
    <div class="main">
        <div class="sidebar">
            <div class="api-status connected" id="apiStatus">✓ 千帆API已连接</div>
            <h3>👥 协作团队</h3>
