...
<head>
    ...
    <title>🧭 指南针工程 - 智能协作平台 V13.1</title>
    <style>
        :root {
            --primary: #165DFF;
...
<body>
    <div class="header">
        <div class="header-left">
            ...
            <div class="logo-sub">智能协作平台 V13.3-120125 <span style="font-size:10px;color:#909399;">(动态意图识别版)</span></div>
        </div>
    ...
    <script>
        ...
        async function fetchStatus() {
            try {
                ...
                // 检测讨论完成
                if (data.isCompleted && timerInterval) {
            ...
            newMessages.forEach((conv, idx) => {
                ...
                msgDiv.innerHTML = `
                    ...
                    <div class="message-content">
                        ...
                        <div class="message-text" id="msg-text-${actualIdx}">${isUser ? conv.content : '<span class="typing-cursor">▊</span>'}</div>
                        ${actionsHtml}
                    </div>
                `;
...
def api_task():
    ...
    if len(schedule_results) > 1:
        ...
        # 任务列表
        task_list = '\n'.join([f"  {i+1}. {n}（预计{t}）" for i, (n, t) in enumerate(zip(task_names, task_times))])
        memory.add_turn('nanqiao', '南乔', 
            f"【智能调度】识别到{len(schedule_results)}个任务类型：\n{task_list}\n\n我将依次引导各Agent完成讨论，请稍候...",
            msg_type='system')
    else:
        ...
        memory.add_turn('nanqiao', '南乔', 
            f"【智能调度】识别任务类型：{schedule_result['task_name']}（置信度{schedule_result['confidence']:.0%}）\n"
            f"主导：{lead_agent_cn} | 参与：{', '.join(participants_cn)} | 预计耗时：{est_time}",
            msg_type='system')
    ...
    def run_multi_task_discussion():
        ...
        for task_idx, schedule_result in enumerate(schedule_results):
            ...
            for agent_key in discussion_flow:
                ...
                if agent_key == lead_agent:
                    ...
                    response += f"\n\n⏱️ 作为{task_name}的主导者，我评估本任务需要 {agent_est} 完成。"
            ...
            if complexity == 'high':
                time.sleep(0.5)
                memory.add_turn('nanqiao', '南乔', '【风险辩论】请各位专家质疑方案风险...', msg_type='system')
            ...
            if total_tasks > 1 and task_idx < total_tasks - 1:
                ...
                memory.add_turn('nanqiao', '南乔', 
                    f"✅ 【任务{current_task_num}完成】{task_name}讨论结束，共识度：{memory.get_consensus_level()}%\n\n➡️ 接下来开始【任务{current_task_num+1}】{next_task}...",
                    msg_type='system')
        ...
        if total_tasks > 1:
            summary = f"🎉 【全部任务完成】\n共完成 {total_tasks} 个任务讨论，整体共识度：{consensus}%\n\n产出物清单：\n" + \
                      '\n'.join([f"  • {r['output_template']}" for r in schedule_results])
        else:
            summary = f"【共识输出】讨论完成。共识度：{consensus}%。产出物：{schedule_results[0]['output_template']}"
...
if __name__ == '__main__':
    print("=" * 60)
    print("🧭 指南针工程 - 智能协作平台 V13 (智能调度版)")
    print("=" * 60)
    print("新增功能：意图识别 | RACI调度 | 三阶段协作")
