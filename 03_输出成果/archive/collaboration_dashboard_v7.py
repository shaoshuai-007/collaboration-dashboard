...
def call_bailian(system_prompt: str, user_message: str, temperature: float = 0.7) -> Optional[str]:
    """调用阿里云百炼API生成回复"""
...
class IntelligentResponder:
    def __init__(self):
        self.api_key_configured = bool(DASHSCOPE_API_KEY)
...
请基于你的角色立场，针对任务发表专业意见。如果有不同意见请明确提出质疑。"""
        ...
        # 调用百炼API
        response = call_bailian(system_prompt, user_message)
        
        if response:
            is_challenge = any(kw in response for kw in ['质疑', '反对', '不同意', '但是', '有问题', '不可行', '建议修改', '成本过高', '周期太长'])
...
<body>
    <div class="header">
        <div>
            <h1>🧭 指南针工程 - 智能协作平台</h1>
            <div class="info">阿里云百炼驱动 · 角色立场辩论 · 智能决策</div>
        </div>
        ...
        async function checkAPIStatus() {
            try {
                ...
                if (data.api_connected) {
                    ...
                    statusDiv.textContent = '✓ 百炼API已连接';
                } else {
...
def api_status():
    """检查API状态"""
    return jsonify({'api_connected': bool(DASHSCOPE_API_KEY)})
...
if __name__ == '__main__':
    print("=" * 60)
    print("🧭 指南针工程 - 智能协作平台 V7 (阿里云百炼版)")
    print("=" * 60)
    print("API状态:", "已连接" if DASHSCOPE_API_KEY else "未配置")
